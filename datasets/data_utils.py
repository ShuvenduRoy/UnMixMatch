...
def split_ssl_data(args, data, target, num_labels, num_classes, index=None, include_lb_to_ulb=True):
    """
    data & target is split into labeled and unlabeld data.
...
def sample_labeled_data(args, data, target,
    ...
    """
    if num_labels == -1:
        return data, target, np.array(range(len(target)))
    ...
    # shuffle so that target classes are running in sequence before repeating
    np_lb_idx = np.array(lb_idx).T.reshape(1, -1).squeeze(0)

    np_lb_idx = np.array(np_lb_idx, dtype=np.int32)
    np.save(dump_path, np_lb_idx)
    return data[np_lb_idx], target[np_lb_idx], np_lb_idx
...
def get_data_loader(dset,
    ...
    if data_sampler is None:
        ...
        try:
            loader = DataLoader(dset, batch_sampler=batch_sampler, num_workers=num_workers, pin_memory=pin_memory)
        except:
            loader = DataLoader(dset, batch_size=batch_size, num_workers=num_workers, pin_memory=pin_memory)
        return loader
